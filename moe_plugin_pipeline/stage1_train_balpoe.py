#!/usr/bin/env python3
"""
Giai đoạn 1: Huấn luyện BalPoE Experts
Mục tiêu: Tạo ra 3 chuyên gia đa dạng và được hiệu chỉnh tốt
...
def main(config):
    """Main training function - copy trực tiếp từ train.py gốc"""
...
if __name__ == '__main__':
    # Argument parsing - copy trực tiếp từ train.py gốc
    args = argparse.ArgumentParser(description='Stage 1: Train BalPoE Experts')
    ...
    print(f"📁 Experts checkpoints: {config.save_dir}")
    print(f"🔧 Tiếp theo: python stage2_optimize_plugin.py --experts_dir {config.save_dir}")